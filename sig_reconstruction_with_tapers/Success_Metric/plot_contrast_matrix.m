...
for i = 1:x
    ...
    if options.changeSensor
        if i == 1
            txt(i) = "No Tapers Added";
        else
            if i-1 >= options.sensorNum
                txt(i) = int2str(options.numOfTapers) + " Tapers Added to Sensor " + int2str(i);
            else
                txt(i) = int2str(options.numOfTapers) + " Tapers Added to Sensor " + int2str(i-1);
            end
        end
    else
        if options.changeNumTapers
            if i == 1
                txt(i) = "No Tapers Added";
            else
                txt(i) = int2str(i-1) + " Tapers Added to Sensor " + int2str(options.tapersSensorNum);
            end
        end
    end
