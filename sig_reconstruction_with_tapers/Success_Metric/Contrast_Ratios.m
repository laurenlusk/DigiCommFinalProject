function [ContrastMatrix, center, REDUCE] = Contrast_Ratios(r,r_est,options,win,spectro,ADJUST)
    ...
    if (ADJUST == 1) % Plot then Adjust
        MAX_PWR = max(spectro.r.p_dB,[],'all');
        [center] = adjust_center(spectro,r,r_est,options,MAX_PWR);
        [f_indx, n_indx, REDUCE] = adjust_boxes(spectro,center,MAX_PWR,REDUCE,TOL,...
                                   r, r_est, options, win);
    else
        N = length(r_est);
    ...
    for i = 1:2:(2*9-1)
        ...
        A = spectro.rError.p(f_indx(i):f_indx(i+1),n_indx(i):n_indx(i+1));
        %%%%%%%%%%%%%%  
